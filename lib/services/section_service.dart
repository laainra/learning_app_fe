...
class SectionService {
  ...
  Future<List<Section>> fetchSections(int courseId) async {
    ...
    final url = Uri.parse('${Constants.baseUrl}/sections?course_id=$courseId');
  ...
  Future<void> addSection(int courseId, String name) async {
    try {
      ...
      final url = Uri.parse('${Constants.baseUrl}/courses/$courseId/sections');
      ...
      if (response.statusCode != 201) {
        throw Exception(
          'Failed to add section. Status code: ${response.statusCode}',
        );
      }
    } catch (e) {
  ...
  Future<void> updateSection(int sectionId, String name) async {
    try {
      final token = await storage.read(key: 'token');
      final url = Uri.parse('${Constants.baseUrl}/sections/$sectionId');
